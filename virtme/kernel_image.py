...
def normalize_kernel_image(arch, path: str, cache_dir: Path, verbose=False) -> str:
    ...
    prepared = _prepare_arm64_image(data, image)
    if prepared is None:
        return path
...
def _prepare_arm64_image(data: bytes, image: Path) -> bytes | None:
    work = _gunzip_if_needed(data)
    linux_section = _pe_section(work, b".linux")
    if linux_section is not None:
        work = _gunzip_if_needed(linux_section)
    ...
    if compression == "zstd":
        return _decompress_zstd(payload, image)
...
def _decompress_zstd(payload: bytes, image: Path) -> bytes:
    try:
        proc = subprocess.run(
            ["zstd", "-q", "-d", "-c"],
            input=payload,
